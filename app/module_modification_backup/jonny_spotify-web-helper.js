...
function SpotifyWebHelper(opts) {
    ...
    this.getCsrfToken = () => {
        ...
        console.log('GETTING CSRF TOKEN RESPONSE');
        console.log(res);
    ...
    this.player.emit('status-will-change', status);
        ...
        if (this.status.playing !== status.playing) {
            if (status.playing) {
                ...
                if (status.track !== undefined && Math.abs(status.playing_position - status.track.length) <= 1) {
  ...
  this.connect = function () {
    ...
    .then(oauthtoken => {
      ...
      console.log('HAS OAUTH TOKEN');
        return this.getCsrfToken();
    })
    .then(csrftoken => {
        this.csrftoken = csrftoken;
      console.log('HAS CSRF TOKEN');
        return getStatus();
    })
    .then(() => {
      console.log('LISTEN');
